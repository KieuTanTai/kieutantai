# My Profile - Kieu Tan Tai

> A passionate developer who loves building meaningful products and exploring new technologies.

[![Portfolio](https://img.shields.io/badge/Portfolio-kieutantai.github.io-4FC08D?style=flat&logo=vuedotjs&logoColor=white)](https://kieutantai.github.io) — Built with Vue.js

---

## GitHub Overview

> A snapshot of my overall GitHub activity — languages, contributions, and more.

![Metrics](/metrics.svg)

---

## My Contribution Journey
>
> Every green square tells a story — consistency, curiosity, and a love for code.
> Here's a full view of my GitHub activity over the years.

![Full Contribution Calendar](/metrics.plugin.calendar.full.svg)

---

## Portfolio Preview

> A live snapshot of my portfolio website.

![Screenshot](/metrics.plugin.screenshot.svg)

---

## Featured Project — Py Smartdocs

> Built with passion for great UI and smooth user experience.

![Contributions - FoodAndDrink](/metrics.plugin.contributors.contributions.svg)

---

## About Me

- Strongest in **C# backend** — building APIs and services with **ASP.NET**
- Also know a bit of the frontend side: **TypeScript**, **React**, **Next.js**
- Currently working on exciting web projects
- Hobby: reading **manga** & **light novels** — always chasing a good story
- Favorite music: *"[Never Stop](https://youtu.be/dH7DQfDAR4A?list=RDdH7DQfDAR4A)"* by **Sawano Hiroyuki[nZk]** — pure energy
- Fun fact: I debug best with a cup of Vietnamese coffee

---

## Now Playing

> Music that keeps me in the zone — anime OSTs, game soundtracks, and beyond.

![Music](/metrics.plugin.music.svg)

---

## Fortune

> A little fortune to brighten your day!

![Fortune](/metrics.plugin.fortune.svg)

---

## Manga Reading List

> My current and favorite manga picks — always chasing a good story!

![Anilist Manga](/metrics.plugin.anilist.manga.svg)

---

_Thanks for visiting my profile! Have a great day_

---

## Credits

Portfolio built using [Vuefolio](https://github.com/ralodev/vuefolio) — a beautiful Vue.js portfolio template by [ralodev](https://github.com/ralodev). Big thanks for the amazing work!
